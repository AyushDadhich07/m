...
def process_document(file_source):
    ...
    Args:
    - file_source (str): The local file path or URL of the document to process.
    ...
    try:
        if file_source.startswith("http://") or file_source.startswith("https://"):
            ...
            logging.info("File downloaded and saved to temporary path: %s", temp_file_path)
        ...
        if 'temp_file_path' in locals():
            try:
                ...
                logging.info("Temporary file %s removed.", temp_file_path)
            except Exception as e:
...
def process_uploaded_document(document_id):
    ...
    # Initialize s3_file_url to handle exceptions correctly
    s3_file_url = None
    ...
    try:
        ...
        # Construct the S3 file URL dynamically
        s3_file_url = f"https://{s3_bucket_name}.s3.{s3_region}.amazonaws.com/{s3_key}"
        logging.info("S3 File URL for document %s: %s", document_id, s3_file_url)

        # Initialize S3 client
        s3 = boto3.client('s3', region_name='eu-north-1')

        # Read the file directly from S3 into memory


        # Process the document content directly from the in-memory file
        texts = process_document(s3_file_url)

        # Use HuggingFace embeddings for vector storage
        embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-mpnet-base-v2")

        # Create a temporary file for the ChromaDB vector store
        ...
        # # Save the vector store to an in-memory file
        # vector_store_buffer = io.BytesIO()
        # vector_store.save(vector_store_buffer)
        # vector_store_buffer.seek(0)  # Rewind the buffer to the beginning
        db = VectorLakeStore.from_documents(documents=texts, embedding=embeddings)
        print()
        print()
        print()
        print()
        ...
        print("db below")
        print(db)
        # Upload the vector store file to S3
        s3_key = f"chroma/{document_id}_vector_store.pkl"
        s3_client.upload_fileobj(db, s3_bucket_name, s3_key)
        ...
        logging.info("Document processing completed successfully for ID: %s with URL: %s", document_id, s3_key)
        return True
